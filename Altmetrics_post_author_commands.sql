CREATE OR REPLACE TABLE "DEV2_LZ"."ALTMETRICS"."ALT_DIM_POST_AUTHORS" (
	...
	ID_ON_SOURCE VARCHAR(300),
	FACEBOOK_WALL_NAME VARCHAR(300),
	TWEETER_ID BIGINT,
	OPS_INSTITUTION_ID BIGINT
);
...
insert into "DEV2_LZ"."ALTMETRICS"."ALT_DIM_POST_AUTHORS"(POST_AUTHOR_ID,
...
(select distinct w.value:author:name as author_name, social_channel,
 ...
 (select v.key as social_channel, v.value as author_arr from "DEV2_LZ"."ALTMETRICS"."T_S_FETCH",
  ...
  "DEV2_LZ"."ALTMETRICS"."ALT_DIM_SOCIAL_CHANNELS" on social_channel_name = social_channel,
  table(getnextval(alt_author_seq)) seq
where author_name is not null and not exists (select POST_AUTHOR_NAME from "DEV2_LZ"."ALTMETRICS"."ALT_DIM_POST_AUTHORS" where
